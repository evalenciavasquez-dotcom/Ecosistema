...
- **Autoría por sombrero**: cada uno de los seis campos sabe si lo redactó el sistema o lo escribió Eduardo, y lo dice en pantalla — solo se marca lo del sistema, porque etiquetar también lo propio hace que se dejen de leer las etiquetas que importan. Editar un texto del sistema lo vuelve de Eduardo: la última mano responde por lo que se está leyendo. Antes esto era una sola marca por ruta, que no se mostraba en ninguna pantalla y además se llenaba mal — un requisito declarado en el tipo y no cumplido en el producto.
